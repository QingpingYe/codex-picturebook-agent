...
**协议版本：** 1.1（2026-09-17 勘误：`docs +fetch`、写后回读校验、双锁治理、首次发布 revision 记录）  
...
2. 该标题必须是最后一个独立章节，前面保留一个空行，后面只能是 `json` 代码块。
...
7. `last_ai_revision_id` 必须是非负整数，表示该目标页最后一次成功 AI 写入后的目标 docx revision。
   - 首次发布新页面时，不能写 `0`。若创建返回 revision `1`，页尾元数据必须写 `1`。
   - 后续 AI 更新时，页尾元数据写入本次更新预期产生的新 revision；索引中的 `last_seen_revision_id` 写入实际返回的新 revision。
...
12. 禁止为了绕开解析错误而重建一个新逻辑键、新页面或新知识库。
...
- [ ] 首次发布页面的页尾 `last_ai_revision_id` 为创建返回的 revision，不得写 `0`。
